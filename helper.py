...
def snn_train_and_save(num_steps, n_first_hidden, num_binary_layers, n_hidden, 
               train_loader, test_loader, save_path, pretrained=False, num_epochs=20, lr=5e-4, weight_decay=0):    
    ...
    if pretrained==True:    
        ...
        train.train_snn(net, train_loader, test_loader, num_epochs = num_epochs, 
                        output='spike', lr=lr, weight_decay=weight_decay)
...
def ann_train_and_save(n_first_hidden, num_hidden_layers, n_hidden, 
               train_loader, test_loader, save_path, pretrained=False, num_epochs=20, lr=5e-4, weight_decay=1e-4):    
    # Create a folder to save the results
    name = 'ann_784-' + str(n_first_hidden)
    ...
    # Randomize the network and train (if no pretrained model is available)
    net = models.ANN(n_first_hidden=n_first_hidden, 
                     num_hidden_layers = num_hidden_layers, n_hidden = n_hidden).to(device)
    f = open(save_path+'results.txt', 'w')
    train.print_and_save('Network architecture: '+name, f)
    f.close()
    print(net)
    ...
    if pretrained==True:    
        ...
        start_time = time.time()
        train.train_ann(net, train_loader, test_loader, num_epochs = num_epochs, lr=lr, weight_decay=weight_decay)
        train_time = time.time()-start_time
        torch.save(net.state_dict(), save_path+'params.pth')
    ...
    file = open(save_path+'results.txt', 'a')
    train_loss, train_acc = train.print_ann_statistics(net, train_loader, file=file, epoch=num_epochs, train=True)
    start_time = time.time()
    test_loss, test_acc = train.print_ann_statistics(net, test_loader, file=file, epoch=num_epochs, train=False)
    test_inference_time = time.time() - start_time
    train.print_and_save(f'Training time: {train_time:.2f} seconds. Test inference time: {test_inference_time:.2f}', file)
    file.close()
    return train_loss, train_acc, test_loss, test_acc, train_time, test_inference_time
...
def compare_snn(variable, train_loader, test_loader, seed = None, num_epochs = 20, 
            save_path = './snn_mnist_comparison/', num_trials=1, lr=5e-4, weight_decay=0):
...
def compare_ann(variable, train_loader, test_loader, seed = None, num_epochs = 20, 
                save_path = './ann_mnist_comparison/', num_trials=1, lr=5e-4, weight_decay=1e-4):
    # Fix a seed in case we want a single fixed trial 
    if (num_trials==1) and (seed is not None):
        torch.manual_seed(seed)
        torch.cuda.manual_seed(seed)
    ...
    for _ in range(num_trials):
        ...
        elif variable == 'num_hidden_layers':
            list_x = [i for i in range(1,7)]
            for num_hidden_layers in list_x:
                list_loss_acc.append(ann_train_and_save(n_first_hidden, num_hidden_layers, n_hidden,
                    train_loader, test_loader, save_path, num_epochs=num_epochs, lr=lr, weight_decay=weight_decay))
            plot_desc = 'Number of hidden layers'
